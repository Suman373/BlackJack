...
let firstCard = Math.floor(Math.random()*(10-1)+1);
let secondCard = Math.floor(Math.random()*(10-1)+1);
...
// new card
function newCard(){
    let newcardel = Math.floor(Math.random()*(10-1)+1);
    sum += newcardel;
...
function replayGame(){
    audio_click.play(); 
    message_change.textContent="Loading..."
    setTimeout(()=>{
